...
class SutdaDeck {
	...
	void registerJokbo() {
		 ...
		 * 문자열로 붙여서 로 점수를 로 저장한다 key , value .
		 */
		jokbo.put("KKKK", 4000);
		jokbo.put("1010", 3100);
		jokbo.put("99", 3090);
	...
	int getPoint(Player p) {
		...
		Integer result = 0;
		/*
		 * (2) . 아래의 로직에 맞게 코드를 작성하시오 1. , jokbo "KK" . 카드 두 장이 모두 광이면 에서 키를 로
		 * 해서 점수를 조회한다 2. (num) jokbo . 두 카드의 숫자 로 에서 등급을 조회한다 3. , . 해당하는 등급이
		 * 없으면 아래의 공식으로 점수를 계산한다 (c1.num + c2.num) % 10 + 1000 4. Player (point)
		 * . 의 점수 에 계산한 값을 저장한다
		 */

		// 카드 두 장이 모두 광이면, KK키로 점수를 조회한다.
		if (c1.isKwang && c2.isKwang) {
			result += (int) jokbo.get("KK");
		} else {
			// 두 카드의 숫자(NUM)으로 족보에서 등급을 조회한다.
			String tmpK = Integer.toString(c1.num) + Integer.toString(c2.num);
			// 조회했는데 없을경우 , 공식으로 점수 계산.
			if (jokbo.containsKey(tmpK))
				result += (int) jokbo.get(tmpK);
			else
...
public class EX11_12 {
	...
	public static void main(String args[]) throws Exception {
		...
		System.out.println();
		System.out.println("1 위는" + rank.firstKey() + "입니다 .");
	}
